...
| Conflict | Fix |
|---|---|
| Filter/router pill row spans the full top of `#graph-panel` | `top: 44px; right: 12px` — drop below the pill row |
| A floating toolbar or legend occupies the top-right | `bottom: 52px; right: 12px` — sits above the status bar |
| Both top-right and bottom-right are taken | `top: 12px; left: 12px` — move to top-left corner |
